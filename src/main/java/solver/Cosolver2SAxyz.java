 ...
 */
public class Cosolver2SAxyz extends CosolverBase {

  public Cosolver2SAxyz() {
    super();
  }

  public Cosolver2SAxyz(TTP1Instance ttp) {
    super(ttp);
  }

  public Cosolver2SAxyz(TTP1Instance ttp, TTPSolution s0) {
    super(ttp, s0);
  }
    ...
    do {
      ...
      do {
        ...
        if (improv1) {
          ...
          ft = sol.ft;
          wc = iBest - 2 < 0 ? 0 : sol.weightAcc[iBest - 2]; // fix index...
          deltaT = 0;
          for (q = iBest - 1; q <= jBest; q++) {
            wc += TwoOptHelper.get2optValue(q, sol.weightRec, iBest, jBest);

            c1 = TwoOptHelper.get2optValue(q, tour, iBest, jBest) - 1;
            c2 = TwoOptHelper.get2optValue((q + 1) % nbCities, tour, iBest, jBest) - 1;
          ...
          Deb.echo("  >>> "+iBest+" & "+jBest);
          Deb.echol("  TACC:");Deb.echo(sol.weightAcc);
          ttp.objective(sol); // TODO avoid this...
          Deb.echol("  TACC:");Deb.echo(sol.weightAcc);
          /* ------------------------------------------------- */
          //ttp.objective(sol); // TODO avoid this...
